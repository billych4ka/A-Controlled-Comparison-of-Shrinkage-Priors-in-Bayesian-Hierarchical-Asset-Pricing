...
Appendix A.7 of the dissertation. They are not unit tests. Each one compares
...
|---|---|---|
| `check_lasso_conditionals.py` | Appendix A.7.1, first paragraph; Table 4.5, row "Analytic conditionals"; supports Section 4.4 | seconds |
| `check_blocked_sur_kronecker.py` | Appendix A.7.1, second paragraph; Section 4.4, the blocked (B, b_bar) update | seconds |
| `check_lasso_blocked_sur.py` | Appendix A.7.1, second paragraph; A.7.2, first paragraph; Table 4.5, rows "Analytic conditionals" and "Distributional moments" | under a minute (60,000 draws) |
| `check_lasso_composition.py` | Appendix A.7.2, second paragraph; A.7.5, first paragraph; Table 4.5, row "Joint composition" | minutes |
| `check_iw_potential.py` | Appendix A.7.3; Section 4.4, the custom differentiable inverse-Wishart density; Section 4.5 | minutes |
| `check_calibration_baseline_gaussian.py` | Table 4.6, Gaussian baseline column; Appendix A.7.4, first paragraph; Section 4.5 | minutes |
| `check_calibration_bayesian_lasso.py` | Table 4.6, Bayesian LASSO column; Appendix A.7.4; Section 4.5; A.7.5, second example | about 3 minutes |
| `check_calibration_horseshoe.py` | Table 4.6, horseshoe column and the three excluded datasets; Appendix A.7.4; A.7.7 via `--full-scale`; Section 4.5 | about 101 minutes |
| `check_calibration_regularised_horseshoe.py` | Table 4.6, regularised horseshoe column; Section 4.5 | 30-40 minutes |
| `check_prior_predictive_horseshoe.py` | Section 4.3, the m_eff = 30.04 discrepancy against the nominal p_0 = 23; Appendix A.7.7, second paragraph; supports Section 4.2.4 | a few seconds |
| `check_horseshoe_prior_tails.py` | Appendix A.7.7, why full-dimensional calibration was abandoned | seconds |
| `check_sparsity_recovery_horseshoe.py` | Section 5.6 and Table 5.11; Appendix A.10, the simulation design; A.7.5, third example | about 1.8 hours (5.3 min per fit, 10 sparse + 10 dense) |
| `check_scale_transportability.py` | Section 4.3, Tables 4.3 and 4.4; Appendix A.8 | about a minute |
| `check_forecast_tests.py` | Appendix A.7.6, size and power of the forecast comparison tests; Section 4.6; validates `src/evaluation/metrics.py` | seconds |
| `check_eq_17_collapse.py` | the disclosed correction to Feng & He's eq. (17) | seconds |
| `check_rhat_ess.py` | Appendix A.6 (script section 7) and Appendix A.7.2 (script section 6); Section 4.5; verifies `src/diagnostics/convergence.py` | seconds |
...
| `check_refit_schedule.py` | Appendix A.3, the refit-schedule check; Section 4.6 | about 20 minutes (60-month only); about 70 minutes with `--schedules 24 60` |
...
in Appendix A (A.6/A.7.2 and A.3 respectively); the same no-drift rule
applies to them.
...
Appendix A.7.8's look-ahead corruption test has no standalone script. It is
implemented as `assert_no_lookahead` in `src/evaluation/backtest.py` and
invoked by all four backtest runners --- `run_backtest_gaussian.py`,
`run_backtest_lasso.py`, `run_backtest_horseshoe.py` and
`run_backtest_regularised_horseshoe.py` --- before the backtest itself, unless
`--skip-lookahead` is passed. It runs as part of the backtest rather than
separately, which is the right place for it. The Gaussian baseline's recorded
check was run standalone at the runner's settings, and its output is appended
to `results/size_bm_25/baseline_gaussian/backtest_log.txt`.
